...
- Log the ordering clearly: "GMInterface | CSS extraction deferred - awaiting class determination"
  ...
  <verify>
Console shows NO "Extracted class: none" logs. Every extraction shows a real class like "pf2e-km".
Only ONE extraction per journal selection (not two).
  </verify>
  <done>
extractedClass is always non-null when extractJournalCSS is called. No duplicate extractions.
  </done>
</task>
...
2. If sheetElement is provided, search for stylesheets IN the sheet:
   ...
   for (const link of linkElements) {
     ...
     // Find this stylesheet in document.styleSheets by href
   ...
   // Also check adopted stylesheets if available
   if (sheetElement.shadowRoot?.adoptedStyleSheets) {
     // Process adopted stylesheets
   }
   ```

3. Add debug logging to show WHERE stylesheets are found:
...
4. Specifically look for premium module stylesheet URLs:
   - modules/pf2e-kingmaker-tools/styles/
   - modules/pf2e-pfs/styles/
   - modules/pf2e-beginner-box/styles/

5. Log when a premium module stylesheet is found:
...
CSS payload is reasonable size (10-50KB, not 270KB).
  ...
  <done>
Premium module stylesheets are found and their rules extracted. CSS payload contains Kingmaker-specific rules.
  </done>
</task>
...
2. Check console for extraction logs:
   - "Extracted class: pf2e-km" (NOT "none")
