...
must_haves:
  ...
  artifacts:
    ...
    - path: "templates/gm-interface.hbs"
      provides: "Participants panel HTML, skill buttons, DC input"
...
Output: Collapsible participants panel with add/remove, skill quick buttons, DC input
</objective>
...
- Empty state shows when no participants
  ...
  <done>Participants panel HTML structure in template with skill buttons and DC input</done>
</task>
   ...
   async _requestSkillCheck(skillSlug, participantIds) {
     ...
     for (const participantId of participantIds) {
       ...
       const request = {
         ...
         participantId,
         skillSlug,
         dc: { value: this.currentDC, visibility: 'gm' }, // Default to secret DC
         timestamp: Date.now()
       };
...
- Check state: `game.storyframe.stateManager.getState().pendingRolls` has entry
  </verify>
  <done>GMInterfaceApp has full participant management and skill request functionality</done>
</task>
...
- Drag-drop zone shows feedback on dragover
  ...
  <done>Participants panel and skill request UI has complete, polished styling</done>
</task>
...
7. "Request Check" and "All" buttons send requests
8. Remove button removes participant from panel
9. Panel collapse/expand works
</verification>
...
- Requests create pending rolls in state
