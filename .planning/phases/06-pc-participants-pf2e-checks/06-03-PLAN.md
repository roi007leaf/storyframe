   ...
   async _prepareContext(options) {
     ...
     if (myParticipant) {
       pendingRolls = (state.pendingRolls || [])
         ...
         .map(r => ({
           ...r,
           skillName: PlayerViewerApp._getSkillDisplayName(r.skillSlug)
         }));
     }
...
3. Add _getSkillDisplayName as a STATIC method (so it can be called from static action handler):
   ```javascript
...
4. Implement _onExecuteRoll action handler (STATIC method - uses PlayerViewerApp.method() for helpers):
   ```javascript
   static async _onExecuteRoll(event, target) {
     ...
     ui.notifications.info(`${PlayerViewerApp._getSkillDisplayName(request.skillSlug)} check complete`);
   }
   ```

5. Add showRollPrompt method (called by socket handler when GM sends roll request):
   ```javascript
...
**Implementation note:** All helper methods called from static action handlers must either be:
- Static methods (called as `PlayerViewerApp._getSkillDisplayName(slug)`)
- Or accessed via the application instance if available

We use static methods here because action handlers in ApplicationV2 are static by convention.
  </action>
  <verify>
Testing requires a GM and player client:
1. GM adds player's PC as participant
2. GM requests Diplomacy check
3. Player viewer re-renders with roll prompt visible (socket handler calls showRollPrompt)
...
6. State shows roll in rollHistory
  ...
  <done>PlayerViewerApp displays roll prompts and executes native PF2e rolls using static methods pattern</done>
</task>
...
- Socket handler (_handlePromptSkillCheck) calls playerApp.showRollPrompt to trigger UI update
