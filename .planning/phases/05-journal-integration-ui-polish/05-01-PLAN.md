...
must_haves:
  ...
  key_links:
    ...
    - from: "storyframe.mjs"
      ...
      via: "hook callback"
      pattern: "Hooks\\.on\\('updateJournalEntry'"
---
...
Add hooks for re-scraping when journal edited:

1. After the ready hook (near bottom), add:
   ```javascript
   // Watch for journal updates - re-scrape styles
...
2. Add renderJournalSheet hook for CSS changes without doc updates:
   ```javascript
...
3. Edit journal while StoryFrame open, verify styles update
4. Select different journal, verify old styles cleared
5. Close StoryFrame, verify no orphan style element in document.head
  </verify>
  <done>
Journal styles extracted, namespaced, and injected into StoryFrame.
Styles re-scrape on journal edit.
...
- [ ] Hooks registered for updateJournalEntry and renderJournalSheet
- [ ] Custom journal styles render in StoryFrame
- [ ] Styles don't affect other Foundry windows
- [ ] Styles update on journal edit
</verification>
...
- Editing journal updates StoryFrame appearance within seconds
</success_criteria>
