...
<task type="auto">
  <name>Task 2: Test drag-drop and apply fixes</name>
  <files>scripts/applications/gm-interface.mjs</files>
  <action>
**Step 1: Test current drag-drop implementation**

Open Foundry and perform these tests, documenting results:
1. Open StoryFrame, select journal with images
2. Drag image from journal content area to speaker gallery
3. Verify: Does dialog appear? Can you enter name? Does speaker get added?
4. Drag actor from sidebar to speaker gallery - verify still works

**Step 2: Document findings in code comment**

At the top of _attachContentImageDrag method, add/update comment:
```javascript
/**
 * Attach drag handlers to images in journal content.
 * Verified 2026-01: Working correctly with current implementation.
 * Test: drag image from content -> dialog -> speaker added
 */
```

**Step 3: Apply fixes if any test failed**

If drag-drop broken, apply these specific fixes:

A. If images not draggable (can't start drag):
   - Ensure img.draggable = true
   - Ensure cursor: grab CSS on .journal-page-content img

B. If drag starts but drop fails:
   - Check dragstart sets data correctly:
     ```javascript
...
C. If drop handler doesn't trigger:
   - Ensure gallery element has ondragover with preventDefault
   - Ensure drop handler parses both 'text/plain' and 'application/json'

D. If dialog doesn't appear:
   - Verify DialogV2 import and usage
   - Check speaker name prompt logic

**Step 4: Enhance visual feedback (only after core works)**

Add CSS to gm-interface.css:
```css
.journal-page-content img {
  cursor: grab;
}
.journal-page-content img:active {
  cursor: grabbing;
}
.storyframe-speakers:has(.drag-over) {
  outline: 2px dashed var(--color-text-hyperlink);
}
```
  </action>
  <verify>
1. Drag image from journal content to speaker gallery - dialog prompts for name
2. Enter name and confirm - speaker appears with image and entered name
3. Drag actor from sidebar to gallery - speaker added with actor image/name
4. Both flows complete without console errors
  </verify>
  <done>
Image drag-drop verified working (with fixes applied if needed).
Actor drag-drop regression-tested and working.
Visual feedback enhanced with grab cursor and drop zone highlight.
  </done>
</task>
